...
CREATE TABLE persona (
    ...
    tipo_documento varchar(120) NOT NULL, -- (c1) antes nullable
...
-- Pendiente de validacion con trigers
...
-- animal ya no tiene id_socio (correccion B2): se retira fk_animal_id_socio.
...
-- detalle_venta -> producto: RESTRICT en vez de CASCADE (correccion B1)
...
-- detalle_carrito -> producto: RESTRICT en vez de CASCADE (correccion B1)
...
-- ix_animal_id_socio retirado: la columna animal.id_socio ya no existe (correccion B2).
...
-- Indices unicos parciales (reglas 'a lo sumo uno' / duplicados con NULL)
...
-- (c5) area: nombre unico entre subareas del mismo padre, y entre raices
...
-- (c5) premio: la posicion (1er, 2do puesto, etc.) no se repite dentro de un mismo concurso
CREATE UNIQUE INDEX uq_premio_concurso_posicion ON premio (id_concurso, posicion) WHERE posicion IS NOT NULL;

-- No existe un entrecruce temporal
